  ...
  return (
    <div className="flex min-h-screen bg-background">
      ...
      <main className="min-w-0 flex-1">
        <header className="border-b border-border bg-background/85 px-6 py-4 backdrop-blur"><Link to="/subhub" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground"><ArrowLeft className="size-4" /> Back to parent products</Link><div className="mt-5 flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">Parent part details</p><h1 className="mt-1 text-2xl font-semibold">{parent.name}</h1><p className="mt-1 text-sm text-muted-foreground">{parent.description}</p></div><div className="rounded-md bg-primary/10 px-3 py-2 text-right"><p className="text-[10px] font-semibold uppercase tracking-wide text-primary">Company variants</p><p className="tabular mt-0.5 text-lg font-semibold text-primary">{parent.variants.length}</p></div></div></header>
