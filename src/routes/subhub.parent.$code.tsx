...
import { ArrowLeft, Boxes, ClipboardCheck, Database, Layers3, LogOut, PackageOpen, Pencil, Plus, Search, Trash2 } from "lucide-react";
  ...
  return (
    <div className="flex min-h-screen bg-white">
      <aside className="flex w-64 shrink-0 flex-col border-r border-sidebar-border bg-white">
        ...
        <nav className="flex-1 p-3"><p className="px-2 pb-2 text-[11px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">Modules</p><Link to="/subhub" className="flex items-center gap-3 rounded-md bg-sidebar-accent px-3 py-2 text-sm font-medium text-sidebar-primary"><Layers3 className="size-4" /> Bills of Materials</Link><Link to="/subhub/raw-materials" className="mt-1 flex items-center gap-3 rounded-md px-3 py-2 text-sm font-medium text-muted-foreground hover:bg-sidebar-accent hover:text-sidebar-primary"><Database className="size-4" /> Raw Materials</Link><div className="group"><Link to="/inventory" className="mt-1 flex items-center gap-3 rounded-md px-3 py-2 text-sm font-medium text-muted-foreground hover:bg-sidebar-accent hover:text-sidebar-primary"><PackageOpen className="size-4" /> Inventory Management</Link><div className="hidden pl-9 group-hover:block"><Link to="/inventory" className="block py-1.5 text-xs text-muted-foreground hover:text-sidebar-primary">Inventory</Link><Link to="/inventory/history" className="block py-1.5 text-xs text-muted-foreground hover:text-sidebar-primary">History</Link><Link to="/inventory/adjustment" className="block py-1.5 text-xs text-muted-foreground hover:text-sidebar-primary">Stock Adjustment</Link></div></div><div className="group"><Link to="/subhub/production" className="mt-1 flex items-center gap-3 rounded-md px-3 py-2 text-sm font-medium text-muted-foreground hover:bg-sidebar-accent hover:text-sidebar-primary"><ClipboardCheck className="size-4" /> Hub Manager</Link><div className="hidden pl-9 group-hover:block"><Link to="/subhub/production" className="block py-1.5 text-xs text-muted-foreground hover:text-sidebar-primary">Production</Link></div></div></nav>
      ...
      <main className="min-w-0 flex-1">
        ...
        <section className="grid gap-6 bg-white p-6 lg:grid-cols-[300px_minmax(0,1fr)]">
          <aside className="panel h-fit overflow-hidden"><div className="border-b border-border px-4 py-4"><div className="flex items-center gap-3"><div className="min-w-0 flex-1"><h2 className="text-sm font-semibold">Company BOM variants</h2><p className="mt-1 text-xs text-muted-foreground">{variants.length} variants</p></div><button type="button" onClick={() => { setEditingVariant(null); setShowVariantCreator(true); }} aria-label="Create variant" className="rounded-md border border-input bg-white p-2 hover:bg-muted"><Plus className="size-4" /></button></div><label className="relative mt-3 block"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><input value={variantSearch} onChange={(event) => setVariantSearch(event.target.value)} placeholder="Search variants" className="h-9 w-full rounded-md border border-input bg-white pl-9 pr-3 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20" /></label></div><div className="space-y-1 p-2">{filteredVariants.map((variant) => <div key={variant.code} className={`relative rounded-md border p-3 transition ${selected.code === variant.code ? "border-primary bg-primary/5" : "border-transparent hover:border-border hover:bg-muted/40"}`}><button type="button" onClick={() => setSelectedCode(variant.code)} className="w-full pr-12 text-left"><p className="tabular text-xs font-semibold text-muted-foreground">{variant.code}</p><p className="mt-1 text-sm font-semibold">{variant.name}</p><p className="mt-1 text-xs text-muted-foreground">{variant.company}</p></button><div className="absolute right-2 top-2 flex gap-0.5"><button type="button" aria-label={`Edit ${variant.name}`} onClick={() => { setEditingVariant(variant); setShowVariantCreator(true); }} className="rounded-md p-1 text-muted-foreground hover:bg-white"><Pencil className="size-3.5" /></button><button type="button" aria-label={`Delete ${variant.name}`} onClick={() => { if (window.confirm(`Delete ${variant.name}?`)) { setVariants((current) => current.filter((item) => item.code !== variant.code)); if (selectedCode === variant.code) setSelectedCode(variants.find((item) => item.code !== variant.code)?.code ?? ""); } }} className="rounded-md p-1 text-muted-foreground hover:bg-white hover:text-destructive"><Trash2 className="size-3.5" /></button></div></div>)}</div></aside>
        ...
        {showVariantCreator ? <VariantDrawer initialVariant={editingVariant} parentName={parent.name} prefix={code} availableParts={availableParts} onClose={() => { setShowVariantCreator(false); setEditingVariant(null); }} onCreate={createVariant} /> : null}
       </main>
    </div>
  );
}

function VariantDrawer({ initialVariant, parentName, prefix, availableParts, onClose, onCreate }: { initialVariant?: Variant | null; parentName: string; prefix: string; availableParts: Part[]; onClose: () => void; onCreate: (variant: Variant) => void }) {
  ...
  const [quantities, setQuantities] = useState<Record<string, number>>(Object.fromEntries((initialVariant?.parts ?? []).map((part) => [part.code, part.qty])));
